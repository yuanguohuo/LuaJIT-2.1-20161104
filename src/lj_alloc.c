...
static void *mmap_map32(size_t size)
{
  dd("Enter");
#if LJ_ALLOC_MMAP_PROBE
  static int fallback = 0;  //Yuanguo: LJ_ALLOC_MMAP_PROBE exists!
  if (fallback)
  {
    dd("Call mmap_probe");
...
#endif
  {
    int olderr = errno;
    /*
     * Yuanguo: MAP_32BIT: Put  the  mapping  into  the  first 2 Gigabytes of the process address space. This flag is supported 
     * only on x86-64, for 64-bit programs. It was added to allow thread stacks to be allocated somewhere in the first 2GB of 
     * memory, so as to improve context-switch performance on some early 64-bit processors. Modern x86-64 processors no longer
     * have this performance problem, so use of this flag is not required on those systems. The MAP_32BIT flag is ignored when
     * MAP_FIXED is set.
     */
...
#if LJ_ALLOC_MMAP32
#define CALL_MMAP(size)		mmap_map32(size)   //Yuanguo: this one!
