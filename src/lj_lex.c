...
int lj_lex_setup(lua_State *L, LexState *ls)
{
  ...
  /**
   * Yuanguo, from wikipedia:
   *
   * The byte order mark (BOM) is a Unicode character, U+FEFF BYTE ORDER MARK (BOM), whose appearance as a magic number at the start of a text stream can 
   * signal several things to a program consuming the text:
   ...
   *
   * UTF-8      : The UTF-8 representation of the BOM is the byte sequence 0xEF,0xBB,0xBF;
   * UTF-16 (BE): 0xFE 0xFF
   * UTF-16 (LE): 0xFF 0xFE
  ...
  if (ls->c == LUA_SIGNATURE[0]) {  /* Bytecode dump. */   //Yuanguo: lua byte code generated by "luac" or "luajit -b"
    //Yuanguo: this is lua byte code, so there should not be UTF-8 BOM or #! header line,
    //if there is, it must be an error!
    if (header) {
      /*
